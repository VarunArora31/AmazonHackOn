  ...
  return (
    <>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.98 }}
        className="group relative flex items-center gap-2 px-4 py-2 rounded-xl bg-white dark:bg-zinc-900 border border-neutral-300 dark:border-white/15 hover:border-neutral-500 dark:hover:border-white/30/70 text-neutral-900 dark:text-neutral-300 hover:text-neutral-700 dark:hover:text-neutral-200 text-xs font-semibold transition-all duration-300 shadow-sm dark:shadow-none hover:shadow-md dark:hover:shadow-[0_0_20px_rgba(255,255,255,0.03)] overflow-hidden"
      >
        {/* Shimmer effect */}
        <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-neutral-500/10 dark:via-white/[0.05] to-transparent" />
        ...
        {isOptimizing && (
          ...
          >
            <motion.div
              ...
              className="relative flex flex-col items-center gap-6 px-10 py-8 rounded-2xl border border-neutral-200 dark:border-white/10 bg-white dark:bg-[#111111]/90 shadow-2xl max-w-sm w-full mx-4"
            >
              {/* Animated orb */}
              <div className="relative w-16 h-16">
                <motion.div
                  ...
                  className="absolute inset-0 rounded-full bg-neutral-300 blur-xl"
                />
                <motion.div
                  animate={{ rotate: 360 }}
                  transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
                  className="absolute inset-2 rounded-full border-2 border-transparent border-t-neutral-700 border-r-neutral-500"
                />
                <div className="absolute inset-0 flex items-center justify-center">
                  <Wand2 className="w-6 h-6 text-neutral-700" />
                </div>
              </div>
              ...
              <div className="h-6 flex items-center">
                <AnimatePresence mode="wait">
                  <motion.p
                    ...
                    exit={{ opacity: 0, y: -8 }}
                    transition={{ duration: 0.3 }}
                    className="text-sm text-neutral-700 dark:text-neutral-300 text-center"
                  >
        ...
        {showProposal && (
          ...
          >
            <motion.div
              ...
              className="relative max-w-md w-full mx-4 rounded-2xl border border-neutral-200 dark:border-white/10 bg-white dark:bg-[#111111] shadow-2xl overflow-hidden"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div className="px-6 pt-5 pb-4 border-b border-neutral-200 dark:border-white/10">
                <div className="flex items-center gap-2.5">
                  <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-neutral-200 border border-neutral-300">
                ...
                {proposedSchedule.map((slot, i) => (
                  <motion.div
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${
                      slot.type === "suggested"
                        ? "bg-neutral-100 border-l-2 border-neutral-400"
                        : "border-l-2 border-neutral-300"
                    }`}
                  >
                    {/* Time */}
                    <span className="shrink-0 w-16 text-[11px] font-mono text-neutral-500 dark:text-neutral-500">
                      {slot.time}
                    </span>
                    ...
                    {/* Type badge */}
                    {slot.type === "suggested" && (
                      <span className="shrink-0 text-[9px] font-semibold text-neutral-700 uppercase tracking-wider px-1.5 py-0.5 rounded bg-neutral-200">
                        AI
                      </span>
                    )}
                  </motion.div>
              ...
              <AnimatePresence>
                {accepted && (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="absolute inset-0 flex items-center justify-center bg-white/95 dark:bg-[#111111]/95 z-10"
                  >
                    <motion.div
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
              ...
              <div className="px-6 py-4 border-t border-neutral-200 dark:border-white/10 flex items-center gap-3">
                <button
                  onClick={handleDiscard}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-transparent border border-neutral-300 dark:border-neutral-800 text-neutral-600 dark:text-neutral-400 text-xs font-semibold hover:border-neutral-400 hover:text-neutral-900 dark:hover:border-neutral-600 dark:hover:text-neutral-300 transition-colors"
                >
                ...
                <button
                  ...
                  disabled={accepted}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-neutral-900 text-white text-xs font-semibold hover:bg-neutral-900 transition-colors disabled:opacity-50 shadow-lg shadow-neutral-900/10"
                >
